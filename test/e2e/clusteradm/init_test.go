...
var _ = ginkgo.Describe("test clusteradm with bootstrap token in singleton mode", func() {
	...
	ginkgo.Context("init cluster manager", func() {
		...
		ginkgo.It("should init multiple times with different flags", func() {
			...
			gomega.Expect(cm.Spec.RegistrationConfiguration.AutoApprovalIdentities[0].Driver).Should(gomega.Equal("csr"))
			gomega.Expect(cm.Spec.RegistrationConfiguration.AutoApprovalIdentities[1].Driver).Should(gomega.Equal("awsirsa"))
			gomega.Expect(cm.Spec.RegistrationConfiguration.AutoApprovalIdentities[0].Identities[0]).Should(gomega.Equal("csr1"))
			gomega.Expect(cm.Spec.RegistrationConfiguration.AutoApprovalIdentities[1].Identities[0]).Should(gomega.Equal("arn:aws:eks:us-west-2:123456789012:cluster/*"))
