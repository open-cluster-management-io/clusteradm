...
func (o *Options) complete(cmd *cobra.Command, args []string) (err error) {
	...
	klog.V(1).InfoS("list options:", "dry-run", o.ClusteradmFlags.DryRun, "clusters", o.clusters)
...
func (o *Options) runWithClient(clusterClient clusterclientset.Interface,
	...
	for _, clusterName := range clusters {
		...
		fmt.Fprintf(o.Streams.Out, "CLUSTERNAME\tADDONNAME\tAVAILABLE\tDEGRADED\tPROGRESSING\n")

		for _, item := range list.Items {
			fmt.Fprintf(o.Streams.Out, "%s\t%s\n", item.GetObjectMeta().GetNamespace(), item.GetObjectMeta().GetName())
		}
