	...
	if o.purgeOperator {
		if !o.ClusteradmFlags.DryRun {
			if err := kubeClient.AppsV1().
				...
				Delete(context.Background(), "klusterlet", metav1.DeleteOptions{}); err != nil {
				return err
			}
			if err := apiExtensionsClient.ApiextensionsV1().
				CustomResourceDefinitions().
				Delete(context.Background(), "klusterlets.operator.open-cluster-management.io", metav1.DeleteOptions{}); err != nil {
				return err
			}
			if err := kubeClient.CoreV1().
				Namespaces().
				Delete(context.Background(), "open-cluster-management-agent", metav1.DeleteOptions{}); err != nil {
				return err
			}
