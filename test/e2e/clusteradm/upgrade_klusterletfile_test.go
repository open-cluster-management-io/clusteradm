...
var _ = ginkgo.Describe("test clusteradm upgrade klusterlet with klusterlet file", ginkgo.Ordered, ginkgo.Label("upgrade-klusterlet-klusterletfile"), func() {
	...
	ginkgo.Context("upgrade klusterlet with klusterlet file", func() {
		ginkgo.It("should upgrade klusterlet with configuration from file", func() {
			...
			gomega.Eventually(func() error {
				err := util.CheckOperatorAndAgentVersion(mcl1KubeClient, "latest")
				if err != nil {
			...
			gomega.Expect(err).NotTo(gomega.HaveOccurred(), "klusterlet upgrade with advanced klusterlet file error")

			ginkgo.By("verify klusterlet was upgraded with advanced configuration from file")
